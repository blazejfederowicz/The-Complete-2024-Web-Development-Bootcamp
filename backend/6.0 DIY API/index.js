...
app.get('/api/users/1/posts',(req,res)=>{
  res.json(users[0].post)
})

app.get('/api/users/1/comments',(req,res)=>{
  res.json(users[0].comments)
})

//2. GET a specific joke
app.get(`/jokes/:id`,(req,res)=>{
  const jokeId = parseInt(req.params.id,10)
  const joke = jokes.find((e)=> e.id === jokeId )
  res.json(joke)
})
//3. GET a jokes by filtering on the joke type
app.get(`/filter`,(req,res)=>{
  const jokeType = req.query.type
  const joke = jokes.filter((e)=> e.jokeType === jokeType )
...
app.post(`/jokes`,(req,res)=>{
  ...
  if(text.trim().length===0 || type.trim().length===0) return res.sendStatus(404);
...
app.put(`/jokes/:id`,(req,res)=>{
  const jokeId = parseInt(req.params.id,10);
  const {text,type} = req.body;
  const index = jokes.findIndex((e)=> e.id===jokeId);
...
app.patch(`/jokes/:id`,(req,res)=>{
  ...
  const joke = {
    id:jokeId,
    jokeText:text || jokes[index].jokeText,
    jokeType:type || jokes[index].jokeType
  }
...
app.delete("/jokes/:id",(req,res)=>{
  const jokeId = parseInt(req.params.id,10);
  if(jokes.find(e=>e.id === jokeId)){
    jokes =jokes.filter(e=> e.id !== jokeId)
    res.sendStatus(200)
  }else{
    res.sendStatus(404)
  }
...
app.delete("/all",(req,res)=>{
  const jokeAuth = req.query.key
  if(jokeAuth === masterKey){
    jokes = []
    res.sendStatus(200)
  }else{
    res.sendStatus(403)
...
async function fetchUserData(userId) {
  try {
    ...
    const [userResponse, postsResponse, commentsResponse] = await Promise.all([
      ...
      fetch(`http://localhost:3000/api/users/${userId}/comments`)
    ]);
    ...
    // You could return a default state or rethrow the error
    return {
      user: null,
      posts: [],
      comments: [],
